...
### 4b. Trade Office Competition
- Trade offices are a contested, shared resource: a Location allows at most **N** `trade_office` buildings across ALL countries (was effectively one-per-country, uncapped per location). Players compete for the limited slots. N = game rule **`ab_trade_office_limit`** (values 2–10 step 1, then 12/14/16/18/20; default 10).
- **Rule → value:** game rule in `main_menu\common\game_rules\ars_belli_rules.txt` (options `trade_offices_2`..`trade_offices_10`, `trade_offices_12/14/16/18/20`), loc `rule_ab_trade_office_limit` / `setting_trade_offices_N` in `ars_belli_rules_l_english.yml`. At game start + monthly, `mp_limits_rules_initialization` (`ars_belli_rules_scripted_effects.txt`) maps the rule flag to global var **`ab_trade_office_limit`** (default 10 set unconditionally first).
- **Build gate** — `REPLACE:trade_office` in `in_game\common\building_types\ars_belli_foreign_buildings.txt` (vanilla def in `<game>\...\building_types\foreign_buildings.txt`; recopy + reapply `allow` on vanilla update). `allow` (location scope): `OR = { any_foreign_buildings_in_location = { building_type = building_type:trade_office owner = scope:actor }  NOT = { any_foreign_buildings_in_location = { count >= global_var:ab_trade_office_limit building_type = building_type:trade_office } } }`. First branch = always allow upgrading your own office here; second = a slot must be free for a NEW office. Blocks new offices only, never level upgrades. Wrapped in `custom_tooltip = { text = ars_belli_trade_office_competition_tt ... }` (loc in `ars_belli_foreign_buildings_l_english.yml`).
- **Yearly pruning** — `yearly_country_pulse` → `ab_trade_office_yearly_pulse` on_action → `ab_trade_office_yearly_cleanup` effect (files: `ars_belli_trade_office_on_actions.txt`, `ars_belli_trade_office_effects.txt`, `ars_belli_trade_office_values.txt`). Runs per-country over `every_owned_location` (each Location has exactly one owner → visited once/yr). For locations with `count > limit` offices, `ab_trade_office_prune_location` destroys the excess via `ordered_foreign_buildings_in_location { order_by = ab_office_owner_market_weakness  max = ab_trade_office_excess_here  location = { destroy_building = prev } }`.
- **"lower trade advantage" = lower `merchant_power_in_market`** of the office owner in the Location's market. `ab_office_owner_market_weakness` (building scope) = `0 - location.market.merchant_power_in_market(owner)` (written `location.market = { subtract = "merchant_power_in_market(prev.owner)" }`); `ordered_*` iterates HIGHEST order_by first → weakest owners destroyed first; `max = count - limit` keeps the strongest `limit`.
- **Why these work:** foreign buildings are one-instance-per-owner-per-location, so counting `trade_office` foreign buildings = counting distinct owner countries. `count`/`order_by`/`max` accept dynamic values (vanilla: `count >= root.modifier:government_size`, `order_by = var:...`); chained data-fn args confirmed (`culture_percentage_in_country(prev.culture)`). `ordered_foreign_buildings_in_location` is unused by vanilla but exists (Jomini auto-generates any/every/ordered/random per list). `max` evaluates in the iterator's parent (location) scope, `order_by` per element (building). **Untested in-game — verify error.log on first run.**
