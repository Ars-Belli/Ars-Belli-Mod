...
- Trade offices are a contested, shared resource: a Location allows at most **N** `trade_office` buildings across ALL countries (was effectively one-per-country, uncapped per location). Players compete for the limited slots. N = game rule **`ab_trade_office_limit`** (values 2–10, default 2).
- **Rule → value:** game rule in `main_menu\common\game_rules\ars_belli_rules.txt` (options `trade_offices_2`..`trade_offices_10`), loc `rule_ab_trade_office_limit` / `setting_trade_offices_N` in `ars_belli_rules_l_english.yml`. At game start + monthly, `mp_limits_rules_initialization` (`ars_belli_rules_scripted_effects.txt`) maps the rule flag to global var **`ab_trade_office_limit`** (default 2 set unconditionally first).
