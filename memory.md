...
### 3h. Sell / Buy Location — fully engine-locked, nothing moddable
...
- **The GUI is not a lever either — proven, do not retry.** The price `economy_slider`'s `max` in `sell_location_action_view.gui` accepts only a literal or a bare getter on the view's own datacontext. Tested in game with four otherwise-identical sliders differing only in `max`: a literal (`9999`) works; `Multiply_float(SellLocationActionView.GetMaxPrice, 0.5)` — arithmetic over vanilla's own known-good source — does **not**; neither does `Player.GetTotalTaxBase` raw nor `FixedPointToFloat(Player.GetTotalTaxBase)`. Since `SellLocationActionView` exposes no tax-base getter (only `CanSend`, `GetAcceptance*`, `GetConfirmMessage`, `GetLocationsSortSearch`, `GetMax/MinPrice`, `GetPossibleLocations`, `GetTotalPrice`, `IsBuying`, `IsSelected`, `OnSend`, `SetTotalPrice`, `ToggleLocation`), no ceiling derived from tax base can be expressed. The mod override was removed; the file is **not** in `replaced_files.txt`.
...
Last updated: 2026-08-13 (economic support cap; union call to arms; tributary transfer bans; sell/buy location proven fully engine-locked incl. the GUI; mercenary listing removed).
