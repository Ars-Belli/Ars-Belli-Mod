...
- Tree shape per branch: adv0 root (`depth = 0`); 1->2 off 0; 3->4->5 off 0. Tree UI still DRAWS branch roots under other nodes (Mercenary root under Court root, Maritime under Trade) even with depth = 0 -> layout only: advance tooltip lists no ADVANCE_REQUIRES_ADVANCE line, only the focus condition. Unconfirmed until a player researches a root without the drawn parent. Real parent via `requires` = real prerequisite; vanilla age roots mostly institution-gated (`has_embraced_institution`), avoid them.
- Event option names dotted (`abm_focus_picks.<branch>`) like all ~14.5k vanilla options; undotted name showed raw key atop option tooltip. Option .tt loc single-line (vanilla has no `\n` in option .tt).
