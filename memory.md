...
- `make_unit_available_for_hire` (generic_action, "Make Available For Hire") is disabled by a **same-name full-file replacement** of `in_game\common\generic_actions\make_unit_available_for_hire.txt` (in `replaced_files.txt`). Renting out own regiments put the mercenary modifiers on the hirer while the owner kept the full hire price — a gold transfer that paid a profit.
