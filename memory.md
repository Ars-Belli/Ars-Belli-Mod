...
- **The kill is one GUI edit.** `ShowCondottieriViewWithFilter` is called from exactly **one** place in the whole game — the "Become Mercenary" button in `in_game\gui\army_builder.gui` — and there is no hotkey for it. The mod's same-name replacement of that file restores `blockoverride "right_widget_visible" { visible = no }` (vanilla's own default from `shared\windows.gui`; vanilla's army_builder empties the override to turn the button on) and empties `blockoverride "right_widget"{}` so the button widget is never built.
