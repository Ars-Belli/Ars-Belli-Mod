...
- Tree shape per branch: adv0 root; 1->2 off 0; 3->4->5 off 0. adv0 MUST have `depth = 0` (vanilla age-root marker): advance w/o requires AND w/o depth gets auto-parented by engine under random node (seen: Mercenary root under Court root -> locked). Don't hang roots under vanilla age roots either: most are institution-gated (`has_embraced_institution`).
