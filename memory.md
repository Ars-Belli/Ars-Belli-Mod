...
- Tree shape per branch: adv0 root (`depth = 0`, confirmed works on NEW game; advance tree changes do not apply to old saves, test on fresh start); 1->2 off 0; 3->4->5 off 0. Leaves (2, 5) have `allow_children = no`: vanilla/country advances with no `requires` get auto-parented by engine onto any leaf (seen: National Assemblies under Court Accounting, NOR advances under Levy/Mercenary leaves). Vanilla age roots mostly institution-gated (`has_embraced_institution`), don't hang branches under them.
