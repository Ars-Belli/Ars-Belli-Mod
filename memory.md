...
- Both pick events = `country_event`, NOT `age_event`. Age window (eventwindow.gui AgeEventWindow) = 3 fixed cards via `AccessOptionItemWithKey('adm'/'dip'/'mil')` + hardcoded Adm/Dip/Mil tooltips; custom options have no key -> every card shows option #1 (tested in game: 3x "Court Focus"). Don't switch back.
