#!/usr/bin/env python3
"""Generate Africa EU5 advances from mapped EU4 national ideas."""
...
IDEA_SET_ALIASES = {"Hadhrami Ideas": "HDR_ideas", "Betsimsaraka Ideas": "malagasy_ideas"}
OUTPUT = Path("in_game/common/advances/abm_f5-t3_africa.txt")
LOC_OUTPUT = Path(
    "main_menu/localization/english/abm_advances_africa_l_english.yml"
)
...
CONVERSIONS: dict[str, tuple[str, ...]] = {
    ...
    "diplomatic_annexation_cost": ("integration_cost_modifier = -0.10",),
    "embargo_efficiency": ("embargo_efficiency = 0.10",),
    "global_ship_cost": ("navy_maintenance_efficiency = 0.10",),
    "global_spy_defence": ("spy_network_defence = 0.20",),
    "light_ship_cost": ("navy_maintenance_efficiency = 0.10",),
    "light_ship_power": ("naval_damage = 0.10",),
    ...
    "trade_range_modifier": ("diplomatic_range_modifier = 0.10",),
    "vassal_income": ("subject_tax_efficiency = 0.10",),
}

IDEA_ASSIGNMENT_OVERRIDES = {
    # Inherited from Middle East generator
    "chagatai_literature": (
        ("legitimacy", "1"),
        ("devotion", "1"),
    ...
    # Africa: nested effects the parser misses
    "malagasy_pirate_ports": (
        ("embargo_efficiency", "0.1"),
        ("privateer_efficiency", "0.15"),
    ),
    "zambezi_maravi_influences": (("stability_cost_modifier", "-0.1"),),
}
...
def read_sheet(args: argparse.Namespace) -> list[MappingRow]:
    ...
    for row in csv.DictReader(io.StringIO(text)):
        ...
        if name in SECTIONS and not tag and not idea_name:
...
def resolve_groups(
    ...
    # Merge groups that resolve to the same EU4 key (e.g. aliases)
    merged: dict[str, tuple[str, str, list[MappingRow]]] = {}
    for (section, idea_name), rows in grouped.items():
        ...
        if key in merged:
            ...
            merged[key] = (section, idea_name, rows)

    resolved = []
    for key, (section, idea_name, rows) in merged.items():
        group = groups[key]
...
def render_outputs(
    ...
    advance_parts = [
        "# Generated by .tools/generate_africa_advances.py",
