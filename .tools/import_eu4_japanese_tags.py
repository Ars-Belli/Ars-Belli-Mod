...
def resolve_style(source: str, country_path: Path, eu5_root: Path) -> CoaStyle | None:
    ...
    if style is None:
        style = CoaStyle(
            emblem=f"ce_mon_{slugify_country_name(country_path)}.dds",
            scale="0.95",
    ...
    return style if emblem_path.is_file() else None
...
def collect_imports(
    ...
    for spec in target_specs:
        ...
        if style is None:
            skipped.append(f"{target}: color imported; no EU5 mon asset found for {source}")
...
def build_coa_output(imports: list[ImportedTag], old_text: str) -> str:
    lines = [
        ...
        "# EU4-derived country flags using native EU5 emblems.",
        "",
    ]
    for imported in imports:
        ...
        lines.extend(["\t}", "}", ""])
    generated_text = "\n".join(lines)
    keys = [imported.coa_key for imported in imports if imported.style is not None]
